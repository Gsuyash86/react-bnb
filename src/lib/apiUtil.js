...
export const ApiUtil = {
  ...
  getProperties(filters) {
    return axios.get('/api/properties', { params: filters }).then((response) => response.data);
  },
  ...
  /**
   * Reserve a property on the specified dates
   * @param {string} propertyId - ID of the property to reserve
   * @param {string} checkinDate - Date of arrival at the property in for format yyyy-mm-dd (e.g. "2022-01-30")
   * @param {number} duration - Number of days to stay at the property. Must be greater than 0.
   * @param {number} guests - Number of guests staying at the property. Must be greater than 0.
   * @return A promise that resolves if the property is successfully reserved.
   */
  reserve(propertyId, checkinDate, duration, guests) {
    return axios
      .post(`/api/properties/${propertyId}/reserve`, { checkinDate, duration, guests })
      .then(() => true);
  },
};
